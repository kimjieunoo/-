...
def main():
    ...
    welcome_words = title_font.render('Earthworm GAME', True, (0, 0, 0), (255, 255, 255))
    ...
    win_words = title_font.render('THE EARTHWORM IS LONG ENOUGH AND YOU WIN!', True, (0, 0, 205), (255, 255, 255))
    screen = pygame.display.set_mode((640, 480), 0, 32)
    pygame.display.set_caption('Earthworm Game')
    new_direction = earthworm.moving_direction
    while 1:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == 27:
                    exit()
                if earthworm.game_started and event.key in key_direction_dict:
                    direction = key_direction_dict[event.key]
                    new_direction = direction_check(earthworm.moving_direction, direction)
            elif (not earthworm.game_started) and event.type == pygame.MOUSEBUTTONDOWN:
                x, y = pygame.mouse.get_pos()
                if 213 <= x <= 422 and 304 <= y <= 342:
                    earthworm.game_started = True
        screen.fill((255, 255, 255))
        if earthworm.game_started:
            earthworm.moving_direction = new_direction  
            earthworm.move_head()
            earthworm.body.append(earthworm.head[:])
            if earthworm.head == earthworm.food:
                if sound:
                    sound_wav.play()
                earthworm.generate_food()
            else:
                earthworm.body.pop(0)
            for seg in earthworm.body:
                pygame.draw.rect(screen, [0, 0, 0], [seg[0], seg[1], 20, 20], 0)
            pygame.draw.rect(screen, earthworm.food_color, [earthworm.food[0], earthworm.food[1], 20, 20], 0)
            if earthworm.check_game_status():
                screen.blit(gameover_words, (241, 310))
                pygame.display.update()
                earthworm = Earthworm()
                new_direction = earthworm.moving_direction
                sleep(3)
            elif len(earthworm.body) == 512:
        ...
        pygame.display.update()
        fps_clock.tick(earthworm.speed)
